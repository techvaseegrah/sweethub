...
const FaceEnrollment = () => {
    ...
    // Check if we're updating an existing enrollment
    const urlParams = new URLSearchParams(location.search);
    const isUpdate = !!urlParams.get('workerId') && urlParams.get('update') === 'true';
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const statusResponse = await axios.get('/admin/attendance/face-status');
                ...
                // Always fetch all workers regardless of service status
                const response = await axios.get('/admin/workers');
                
                // If workerId is provided for update, show all workers and pre-select the worker
                // Otherwise, show only unenrolled workers (for new enrollment)
                if (isUpdate && workerIdFromUrl) {
                    setWorkers(response.data);
                    // Find the worker with the provided ID
                    const worker = response.data.find(w => w._id === workerIdFromUrl);
                    if (worker) {
                        setSelectedWorker(workerIdFromUrl);
                        setMessage({ 
                            type: 'info', 
                            text: `Updating face enrollment for ${worker.name}. Please upload or capture new images.` 
                        });
                    }
                } else {
                    // Show only unenrolled workers for new enrollment
        ...
        fetchData();
    }, [location.search, isUpdate, workerIdFromUrl]);
    ...
    const captureAndRecognize = useCallback(async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                const response = await axios.post('/admin/attendance/recognize-face', {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const successMessage = isUpdate 
                ? `${enrolledWorker?.name} face enrollment updated successfully! Now mark attendance by looking at the camera.`
                : `${enrolledWorker?.name} enrolled successfully! Now mark attendance by looking at the camera.`;
            ...
            // Update workers list - only for new enrollments, not updates
            if (!isUpdate) {
                const response = await axios.get('/admin/workers');
                const unenrolled = response.data.filter(w => !w.faceImages || w.faceImages.length === 0);
                setWorkers(unenrolled);
    ...
    const resetToEnrollment = () => {
        if (isUpdate) {
            // If this was an update, close the window since it was opened in a new tab
            window.close();
        } else {
            // Otherwise, reset to the normal enrollment state
            ...
            {enrollmentSuccess ? (
                ...
                <>
                    <div className="flex items-center justify-between mb-6">
                        ...
                        >
                            <LuArrowLeft className="mr-2" /> {isUpdate ? 'Back to Workers' : 'Back to Enrollment'}
                        </button>
                    </div>
                    
                    <p className="text-center text-gray-600 mb-6">
                        {isUpdate 
                            ? 'Face enrollment updated successfully! Now position your face in the camera frame to mark your attendance.' 
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            ...
                            <select
                                ...
                                disabled={isLoading || (isUpdate && workerIdFromUrl)}
                            >

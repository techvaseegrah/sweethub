...
const ExpenseHistory = () => {
  ...
  useEffect(() => {
    ...
    if (shopFilter) {
      result = result.filter(expense => {
        // Admin expenses have admin field, shop expenses have shop field
        if (expense.admin) {
          // This is an admin expense, only show if "Admin" is selected
          return shopFilter === 'admin';
        } else if (expense.shop) {
          // This is a shop expense, only show if the shop matches
          return expense.shop._id === shopFilter;
        }
        return false;
      });
    }
  ...
  }, [categoryFilter, dateFrom, dateTo, paymentModeFilter, shopFilter, expenses]);
  ...
  const downloadDateRangeReport = () => {
    ...
    const filterInfo = {};
    if (shopFilter) {
      const shop = shops.find(s => s._id === shopFilter);
      filterInfo.shop = shop ? shop.name : (shopFilter === 'admin' ? 'Admin Expenses' : shopFilter);
    }
    if (categoryFilter) filterInfo.category = categoryFilter;
    if (paymentModeFilter) filterInfo.paymentMode = paymentModeFilter;
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {expense.admin ? (
                          ...
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                            {expense.shop?.name || 'Unknown Shop'}
                          </span>
                        )}
                      </td>

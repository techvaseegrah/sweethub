...
function CreateBill({ baseUrl = '/admin' }) {
  ...
  // Auto focus on customer mobile number when component loads or navigates to create bill
  useEffect(() => {
    if (!isEditMode) {
      setTimeout(() => {
        // Find the scrollable container and scroll it to top
        const scrollContainer = document.getElementById('create-bill-container');
        if (scrollContainer) {
  ...
  useEffect(() => {
    if (location.state && location.state.billData && location.state.isEditMode) {
      ...
      if (baseUrl === '/admin' && bill.shop) setSelectedShop(bill.shop._id || bill.shop);
      if (bill.fromInfo) setFromInfo(bill.fromInfo);
  ...
  const fetchProductsAndShops = useCallback(async () => {
    setLoading(true);
    try {
      const gstRes = await axios.get('/admin/settings/gst');
      ...
      if (baseUrl === '/admin') {
        ...
        // Fetch workers based on selected shop
        if (selectedShop && selectedShop !== 'admin') {
          promises.push(axios.get(`/admin/workers?shopId=${selectedShop}`, { withCredentials: true }));
        } else {
          promises.push(axios.get('/admin/workers', { withCredentials: true }));
        }
      }

      const responses = await Promise.all(promises);
      setProducts(responses[0].data);
      if (responses.length > 1) setShops(responses[1].data);
      if (responses.length > 2) setWorkers(responses[2].data);
  ...
  useEffect(() => {
    if (selectedShop && selectedShop !== 'admin' && shops.length > 0) {
      const selectedShopObj = shops.find(shop => shop._id === selectedShop);
      if (selectedShopObj) {
        setFromInfo({
          name: selectedShopObj.name,
          address: selectedShopObj.location || '',
          gstin: selectedShopObj.gstNumber || '',
          state: 'Tamil Nadu',
          stateCode: '33',
          phone: selectedShopObj.shopPhoneNumber || '',
          email: selectedShopObj.email || ''
        });
      }
  ...
  const handlePrintBill = (billData = null, forcePrint = false) => {
    ...
    import('../../../utils/generateBillPdf').then(({ printBill, generateBillPdf }) => {
      ...
      const billDataFormatted = {
        ...
        customerMobileNumber: dataToPrint.customerMobile,
  ...
  const handleSubmit = async (e, forcePrint = false) => {
    ...
    if (!customerMobileNumber) return setError('Enter Mobile Number');
    if (billItems.length === 0) return setError('Add items first');
    ...
    try {
        ...
        const url = isEditMode ? `${baseUrl}/bills/${originalBillId}` : `${baseUrl}/bills`;
  ...
  useEffect(() => {
    const handleKey = (e) => {
      ...
      if (e.ctrlKey && e.shiftKey && e.key === 'Enter') handleSubmit(e, true); 
      ...
      if (e.key === 'F2') {
        e.preventDefault();
        const newWindow = window.open(`${window.location.origin}${baseUrl}/bills/create`, '_blank');
  ...
  return (
    // CHANGE 1: Use h-screen with overflow-y-auto on the wrapper itself. 
    // This creates a scrollbar for this container specifically, ignoring parent limits.
    <div id="create-bill-container" className="bg-gray-100 h-screen flex flex-col font-sans overflow-y-auto overflow-x-hidden">
        <style>{`
                ...
                <div className="flex items-center gap-4">
                     {baseUrl === '/admin' && (
                        <select 
                            ...
                            className="border rounded p-1 bg-gray-50 text-sm font-bold"
                        >
                            <option value="admin">Admin (Factory)</option>
                 ...
                 <div className="flex-1 max-w-2xl">
                     <div className="grid grid-cols-[1fr_auto] gap-y-2 items-center">
                         ...
                         <div className="col-span-2 flex justify-end gap-2 mt-2">
                             ...
                             <button ref={createBillButtonRef} onClick={handleSubmit} className="px-8 py-2 bg-blue-500 text-white rounded font-bold hover:bg-blue-600 shadow-sm">Save</button>
                         </div>

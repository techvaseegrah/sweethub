...
function StockAlerts({ baseUrl = '/admin' }) {
  ...
  useEffect(() => {
    const calculateAllAlerts = async () => {
      try {
        const allProductsResponse = await axios.get(PRODUCTS_URL, { params: { fetchAll: true }, withCredentials: true });
    ...
    <div className="bg-white p-4 md:p-6 rounded-xl shadow-md">
      ...
      {baseUrl === '/admin' && (
        <div className="mb-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          <div
            className={`p-4 rounded-lg shadow-sm flex flex-col items-center justify-center cursor-pointer transition-all duration-200 ${selectedShop === 'admin' ? 'bg-blue-100 border-2 border-blue-500' : 'bg-gray-50 hover:bg-gray-100'
              }`}
          ...
          {shops.map(shop => (
            <div
              key={shop._id}
              className={`p-4 rounded-lg shadow-sm flex flex-col items-center justify-center cursor-pointer transition-all duration-200 ${selectedShop === shop._id ? 'bg-blue-100 border-2 border-blue-500' : 'bg-gray-50 hover:bg-gray-100'
                }`}

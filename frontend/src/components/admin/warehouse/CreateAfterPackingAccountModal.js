...
function CreateAfterPackingAccountModal({ onClose, onAccountCreated, editingAccount = null, showManageAccountsInitial = false }) {
    ...
    const fetchAccounts = async () => {
        try {
            const response = await axios.get('/admin/after-packing-only-users', {
                withCredentials: true,
            });
            setAccounts(response.data);
        } catch (err) {
            console.error('Error fetching after-packing-only users:', err);
            setError('Failed to load after-packing-only accounts');
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (currentEditingAccount) {
                ...
                await axios.put(`/admin/after-packing-only-users/${currentEditingAccount._id}`, {
                ...
                setSuccess('After packing-only account updated successfully!');
            } else {
                // Create new account
                await axios.post('/admin/after-packing-only-users', {
                    username: formData.username,
                ...
                setSuccess('After packing-only account created successfully!');
            }
            ...
            console.error('Error saving after-packing-only user:', err);
            if (err.response?.data?.message) {
                setError(err.response.data.message);
            } else {
                setError(currentEditingAccount ? 'Failed to update after packing-only account. Please try again.' : 'Failed to create after packing-only account. Please try again.');
            }
        } finally {
            setLoading(false);
        }
    ...
    const confirmDelete = async () => {
        ...
        try {
            await axios.delete(`/admin/after-packing-only-users/${accountId}`, {
                withCredentials: true,
            });
            
            setSuccess('After packing-only account deleted successfully!');
            fetchAccounts(); // Refresh the list
            setDeleteConfirmState({ show: false, accountId: null }); // Close confirmation
        } catch (err) {
            console.error('Error deleting after-packing-only user:', err);
            setError('Failed to delete after packing-only account. Please try again.');
            setDeleteConfirmState({ show: false, accountId: null }); // Close confirmation
        }
    };
            ...
            {showManageAccounts ? (
                <div>
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-semibold">Manage After Packing Accounts</h3>
                        <button
                            onClick={() => {
                                setShowManageAccounts(false);
                                        ...
                                        <td className="py-2 px-4 border-b text-sm text-gray-500">
                                            {account.allowedCategories && account.allowedCategories.length > 0 ? (
                                                <div className="flex flex-wrap gap-1">
                                                    {account.allowedCategories.map(cat => (
                                                        <span key={cat?._id || Math.random()} className="bg-green-100 text-green-800 text-[10px] px-1.5 py-0.5 rounded-full font-medium border border-green-200">
            ...
            {deleteConfirmState.show && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-lg p-6 max-w-md w-full">
                        ...
                        <p className="mb-6">Are you sure you want to delete this after packing account? This action cannot be undone.</p>
...
export default CreateAfterPackingAccountModal;
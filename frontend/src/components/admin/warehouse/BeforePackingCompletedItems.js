...
const BeforePackingCompletedItems = () => {
    ...
    const fetchItems = useCallback(async () => {
        setLoading(true);
        try {
            // Fetch all before packing items and filter for completed ones
            ...
            setError('Failed to fetch Before Packing completed items.');
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-md">
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl font-bold">Before Packing - Completed Items</h1>
            </div>
            <p className="text-gray-600 mb-6">View all completed items in the Before Packing stage.</p>
...
export default BeforePackingCompletedItems;
...
function CreateBill({ baseUrl = '/shop' }) {
  ...
  // State for FROM and TO information (Optional as per requirement)
  ...
  useEffect(() => {
    const savedFromInfo = localStorage.getItem('shopBillingFromInfo');
    if (savedFromInfo) {
      try {
      ...
      setFromInfo({
        name: 'The Sweet Hub',
        address: 'Bharath petrol bank, TK, opp. Thuraiyur Main Road, Thuraiyur, Tamil Nadu 621010',
  ...
  useEffect(() => {
    ...
    if (fromInfo.name || fromInfo.address || fromInfo.gstin || fromInfo.state || 
      ...
      localStorage.setItem('shopBillingFromInfo', JSON.stringify(fromInfo));
    }
  }, [fromInfo]);

...
// Helper function to check if face recognition service is available (React-based)
const isFaceRecognitionAvailable = async () => {
    // Since we're using React-based face recognition with face-api.js,
    // the service is always available on the frontend
    return true;
};
...
// --- Face Recognition and Attendance Logic (React-based) ---
exports.recognizeFaceForAttendance = async (req, res) => {
    ...
    try {
        ...
        if (!bestMatch) {
            cleanupFile();
            return res.status(404).json({ message: 'Face not recognized. Please enroll an admin worker first or try again.' });
        }
...
const handleAttendancePunch = async (workerId) => {
    ...
    const worker = await Worker.findOne({ _id: workerId, shop: { $exists: false } }).populate('shift');
    if (!worker) {
        throw new Error('Worker not found or not authorized for admin attendance.');
    }
...
// Helper function to check if worker has an incomplete attendance record from yesterday
const checkForIncompleteYesterdayRecord = async (workerId) => {
    const today = new Date();
    const startOfToday = new Date(today.getFullYear(), today.getMonth(), today.getDate());
...
exports.getMonthlyAttendance = async (req, res) => {
    try {
        ...
        attendanceRecords.forEach(record => {
            // Check if worker exists (not null) and has an _id before accessing its properties
            if (record.worker && record.worker._id) {
                try {
                    const workerId = record.worker._id.toString();
                    if (!attendanceByWorker[workerId]) attendanceByWorker[workerId] = [];

...
// Create a new attendance-only user (shop admin only)
exports.createAttendanceOnlyUser = async (req, res) => {
    ...
    try {
        ...
        const newUser = new User({
            ...
            shop: req.shopId, // Associate the user with the current shop
        });
...
// Get all attendance-only users (shop admin only)
exports.getAllAttendanceOnlyUsers = async (req, res) => {
    try {
        ...
        // Only return attendance-only users associated with the current shop
        const users = await User.find({ 
            role: attendanceOnlyRole._id,
            shop: req.shopId  // Only users associated with current shop
        })
...
// Update an attendance-only user (shop admin only)
exports.updateAttendanceOnlyUser = async (req, res) => {
    const { id } = req.params;
    const { name, username, password } = req.body;

    try {
        // Only find the user if they belong to the current shop
...
// Delete an attendance-only user (shop admin only)

...
exports.updateShopProduct = async (req, res) => {
  try {
    ...
    if (updateData.category !== undefined) {
      product.category = updateData.category === '' ? null : updateData.category;
    }
    ...
    // Handle mixed sweet components update
    if (product.isMixedSweet && updateData.components && Array.isArray(updateData.components)) {
      try {
        // Find the latest production record for this mixed product
        const latestProduction = await MixedSweetProduction.findOne({ mixedProductId: id, shop: shopId })
          .sort({ createdAt: -1 });

        if (latestProduction) {
          latestProduction.components = updateData.components;
          // Synchronize name and SKU with the product
          latestProduction.name = product.name;
          latestProduction.sku = product.sku;
          await latestProduction.save();
        }
      } catch (err) {
        console.error('Error updating mixed sweet production record:', err);
        // We don't fail the whole update just because history record update failed, 
        // but normally it should succeed if the product exists.
      }
    }
...
exports.createShopProduct = async (req, res) => {
  try {
    ...
    if (existingProduct) {
      ...
      existingProduct.stockLevel = newStockLevel;
      if (stockAlertThreshold !== undefined) existingProduct.stockAlertThreshold = parseFloat(stockAlertThreshold);
      if (prices && Array.isArray(prices)) existingProduct.prices = prices;
